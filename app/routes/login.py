...
# 建立一個輔助函式，只有在真正需要用到 Redis 時才建立連線
...
def login_form(request: Request):
    ...
    try:
        requests.get(f"{AUTH_SERVICE_URL}/healthz", timeout=0.5)
...
def login(request: Request, username: str = Form(...), password: str = Form(...)):
    try:
        ...
        # 這邊呼叫 get_redis_client()，避免開機時卡住
        redis_client = get_redis_client()
        redis_client.setex(f"session:{token}", 3600, username)

        resp = RedirectResponse("/dashboard", status_code=302)
        
        # 既然登出有 delete_cookie，登入成功時記得把 Cookie 寫進去瀏覽器
